...
A tabela a seguir resume as principais categorias de modelos matematicos usados neste livro:

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Categoria} & \textbf{Estrutura matematica} & \textbf{Exemplo tipico} \\
\midrule
Algebrico & equacoes sem derivadas & balanco de massa, lei de Hooke \\
EDO & derivadas no tempo & crescimento populacional, circuito RC \\
EDP & derivadas no tempo e espaco & difusao de calor, vibracao \\
Estocastico & variaveis aleatorias & ruido, filas, Monte Carlo \\
\bottomrule
\end{tabular}
\end{center}
...
% EXEMPLO TIPO PROVA
% =====================================================================
\section{Exemplo tipo prova: identificacao de variaveis e hipoteses}

\begin{problem}
Uma bola de borracha e lancada verticalmente para cima com velocidade inicial $v_0$.
Ela sofre resistencia do ar proporcional a sua velocidade.
\begin{enumerate}
    \item Identifique as variaveis dinamicas e os parametros do problema.
    \item Classifique o modelo matematico quanto a sua natureza.
    \item Escreva a equacao governante para a fase de subida.
    \item Cite tres hipoteses explicitamente assumidas.
\end{enumerate}
\end{problem}

\textbf{Solucao}:

\textbf{1. Variaveis e parametros}:
\begin{itemize}
    \item Variaveis dinamicas: posicao $y(t)$ e velocidade $v(t) = \dot{y}$.
    \item Parametros: massa $m$, aceleracao gravitacional $g$, coeficiente de arrasto $b>0$.
\end{itemize}

\textbf{2. Classificacao}: modelo continuo, dinamico, deterministico, governado por uma
EDO de primeira ordem na velocidade (ou segunda ordem na posicao).

\textbf{3. Equacao governante}:

Aplicando a Segunda Lei de Newton na fase de subida ($v>0$):
\begin{equation}
m\frac{dv}{dt} = -mg - bv, \qquad v(0) = v_0 > 0.
\end{equation}
Reescrevendo em termos de posicao: $m\ddot{y} = -mg - b\dot{y}$.

\textbf{4. Hipoteses}:
\begin{enumerate}
    \item Arrasto linear em $v$ (regime laminar, valido para Reynolds baixo).
    \item Gravidade $g$ constante (valido para $h \ll R_{\text{Terra}}$).
    \item Massa $m$ constante ao longo do movimento.
\end{enumerate}

\begin{remark}
Esse e o tipo de questao que aparece no inicio das provas: apresentar uma situacao
fisica e pedir que o aluno identifique variaveis, parametros, tipo de modelo e
equacao governante. A diferenciacao entre \emph{variavel} (muda com o tempo) e
\emph{parametro} (fixo no contexto do modelo) e frequentemente cobrada.
\end{remark}
...
\section{Ciclo iterativo de modelagem}

Na pratica, modelar nao e linear. E um ciclo de refinamentos sucessivos.
Dym descreve esse processo como o nucleo da modelagem matematica:

\begin{enumerate}
    \item \textbf{Hipoteses iniciais}: simplifique ao maximo para comecar.
    \item \textbf{Formulacao}: escreva as equacoes baseadas nas hipoteses.
    \item \textbf{Solucao}: resolva analitica ou numericamente.
    \item \textbf{Comparacao com dados}: valide o modelo contra observacoes.
    \item \textbf{Revisao}: se necessario, retorne ao passo 1 com hipoteses mais refinadas.
\end{enumerate}

A chave e saber quando parar: um modelo e bom o suficiente quando atende ao objetivo com custo aceitavel.

\begin{remark}
Comecar pelo modelo mais simples possivel (modelo minimo viavel) e uma estrategia poderosa.
Adicionar complexidade so e justificado se houver ganho mensuravel na precisao ou no entendimento.
\end{remark}

\subsection{Exemplo: modelo de queda livre revisitado}

No modelo de queda livre, a sequencia e:
\begin{enumerate}
    \item Hipotese inicial: sem resistencia do ar. Modelo: $\ddot{y} = -g$.
    \item Solucao: $y(t) = h - \tfrac{1}{2}gt^2$.
    \item Validacao: bom para objetos densos em baixas velocidades.
    \item Refinamento: adicionar arrasto $F_d = -bv$. Novo modelo: $m\ddot{y} = -mg - b\dot{y}$.
    \item Segunda validacao: melhor para altas velocidades ou objetos leves.
\end{enumerate}

\section{Conexao entre os capitulos}

Este livro e organizado como uma sequencia de ferramentas que se complementam:

\begin{itemize}
    \item \textbf{Capitulo 2 (Analise Dimensional)}: antes de resolver, verifique se as dimensoes fazem sentido
          e reduza o numero de parametros pelo Teorema Pi.
    \item \textbf{Capitulo 3 (Escala e Fractais)}: entenda como o comportamento muda com o tamanho;
          leis de potencia $Y \propto X^\alpha$ identificam os mecanismos dominantes.
    \item \textbf{Capitulo 4 (Serie de Taylor)}: linearize equacoes nao lineares perto de equilibrio
          para analise de estabilidade e simplificacao.
    \item \textbf{Capitulo 5 (Comportamento Exponencial)}: solucao analitica da classe mais frequente
          de EDOs, incluindo crescimento, decaimento e modelos populacionais.
\end{itemize}

\begin{remark}
Os quatro capitulos formam um kit de ferramentas complementar: analise dimensional prepara o modelo,
escala identifica termos dominantes, Taylor lineariza e o comportamento exponencial resolve a EDO resultante.
\end{remark}
